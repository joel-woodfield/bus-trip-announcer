...
class TripAnnouncer:
    """
    Keeps track of what the next stops are and the time until the bus reaches those stops (StopTime objects).

    The method update_next_stop_times must be called to update the information as the bus travels.

    Attributes
    ----------
    _next_stops_finder: NextStopsFinder
        the NextStopsFinder object for the announcer to retrieve the next stops from
    ...
    def __init__(self, next_stops_finder: NextStopsFinder):
        """
        Initializes the announcer with a NextStopsFinder object to be able to retrieve the next stops.
        :param next_stops_finder: the NextStopsFinder object for the announcer to retrieve the next stops from
        """
    ...
    def update_next_stop_times(self, location: RouteLocation) -> None:
        """
        Updates the next stop times the announcer is keeping track of for the new route location.
